...
#ifdef CONFIG_FH8626V100_SD0_1BIT
...
/*
 * FH8626V100 SD0 data for the stock one-bit wiring. A zero caps field
 * selects one-bit mode. Card detect comes from the controller's dedicated
 * SD0_CD input and SD0_1BIT_NO_WP leaves the remaining data pads untouched.
 */
...
/*
 * Keep RTC registration optional in the generic FH8626V100 platform.
 * Boards without a usable RTC clock/backup domain leave the option disabled.
 */
#ifdef CONFIG_FH8626V100_ONCHIP_RTC
...
/* Six-channel priority policy recovered from the stock kernel. */
 ...
 * Device profiles may use that userspace ABI for board-specific mux changes.
 */
...
#ifdef CONFIG_FH8626V100_SD0_1BIT
	/* One-bit SD0 leaves the unused data pads available to the board. */
